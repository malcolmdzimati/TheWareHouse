...
enter_into_div_new=function(name, developer, url, genre, platform, date, pic, divpar)
{
       var txt2 = "<div class='gamect'><a id=gname>"+name+"</a>";
           ...
           txt2 += "<li>Game URL:<a href='" + url + "' class='gameurl'> Website</a></li>";
...
var iload = {
    "url": "https://cors-anywhere.herokuapp.com/https://www.freetogame.com/api/games?sort-by=release-date",
    "method": "GET",
    "timeout": 0,
    "headers": {
        "Content-Type": "json",
...
ajax1 = function(aja)
{
  $(document.getElementById('newct')).empty();
  console.log("start");
 return $.ajax(aja).done(function(response){
   loadgames(response);
   $(document.getElementById("loadingnr")).fadeOut("slow");
  });
}
...
loadgames = function(games){
       ...
       {
       enter_into_div_new(games[i].title, games[i].developer, games[i].game_url,
                             games[i].genre, games[i].platform, games[i].release_date,
                             games[i].thumbnail, document.getElementById('newct'));
       }
}
